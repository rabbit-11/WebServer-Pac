define(function () {
    ...
    function initstate() {
        ...
        EventUtil.showOrHide(btn_end, 1 );
    ...
    function loaddata(data) {
        ...
        $.get(webglobal.templates.mod_followupvisit_detail, { stamp: Math.random() + 1 }, function (template) {
            var arr = [];
    ...
    function show(ctrl_wrapper) {
        require(["share_services", "base_usercontrol", "tmpl", "jquery", "common", "jextend", "web_global", "easyui"], function (s, b) {
            ...
            var web_list_err_callback = function (data) {console.log(data) };
            //获取参数
            var _req = myrequest.getrequest();

            var dictionary = new myextend.Dictionary();
            dictionary.set("generalinfoid", 1);
            //myextend.ajaxPost_simple(webglobal.services.GetPregnantInfo, dictionary, web_list_callback, true);
            myextend.ajaxPost(webglobal.services.PostReturnVisit, dictionary, web_list_callback, web_list_bef_callback, web_list_com_callback, web_list_err_callback, true);
    ...
    function guiddata() {
        ...
        data.code="200";
        data.info=allitem.getItems();
        data.info.id=1;
        data.info.updatetime=(new Date()).Format("yyyy-MM-dd hh:mm:ss.S");
        if(base.getVal('tmpl_createtime')=="")
            data.info.createtime=(new Date()).Format("yyyy-MM-dd hh:mm:ss.S");
        data.msg=null;
    ...
    function setPersonInfo(){
        require(["share_services", "base_usercontrol", "tmpl", "jquery", "common", "jextend", "web_global", "easyui"], function (s, b) {
            var web_list_callback = function (data) {
                ...
                var _req = myrequest.getrequest();
                setRecordTime();
                setInspectionDoctor(_req.doctorname);
                setVisitDate();
    ...
    function setVisitDate(){
        base.setattr(getCtrl("[data-id='tmpl_visitdate']"),"data-value",(new Date()).Format("yyyy-MM-dd")); 
    }
    function setInspectionDoctor(data){
        base.setattr(getCtrl("[data-id='tmpl_inspectiondoctor']"),"data-value",data);
    }
    function setCreateTime(){
        if(base.getVal('tmpl_createtime')=="")
        base.setVal('tmpl_createtime',(new Date()).Format("yyyy-MM-dd hh:mm:ss.S"));
    }
    function setUpdateTime(){
        base.setVal('tmpl_updatetime',(new Date()).Format("yyyy-MM-dd hh:mm:ss.S"));
    }
    ...
    function setRecordTime(){
        base.setattr(getCtrl("[data-id='tmpl_recordtime']"),"data-value",(new Date()).Format("yyyy-MM-dd")); 
    }
    return {
        show: show,
        setBMI: setBMI,
