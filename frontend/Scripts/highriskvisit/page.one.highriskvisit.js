define(function () {
    var wrapper_name = "#win_mod_one_highriskvisit_wrapper";
    function using(callback) {
        require(["base_usercontrol", "tmpl", 'jquery', "jextend", "easyui", "common"], function (b) {
            class_base = b;
    ...
    function show() {
        using(function () {
            var id = geturlparam("chaseid");
            ReadOnehighriskvisit();
        });
    }
    ...
    function loaddata(data, mod, ctrl_wrapper) {
        ...
        arr.push(data); 
        $(ctrl_wrapper).html("");
        $.get(webglobal.templates.mod_one_highriskvisit, { stamp: Math.random() + 1 }, function (template) {
    ...
    function ReadOnehighriskvisit() {
        ...
        dictionary.set("chaseid", geturlparam("chaseid"));
        dictionary.set("visitrecordid", geturlparam("visitrecordid"));
        dictionary.set("pregnantinfoid", geturlparam("pregnantinfoid"));
        dictionary.set("attached_type", geturlparam("attached_type"));
        dictionary.set("chasedoctorcode", geturlparam("chasedoctorcode"));
        dictionary.set("chasedoctorname", geturlparam("chasedoctorname"));
        dictionary.set("jigoudm", geturlparam("jigoudm"));
        dictionary.set("personname", geturlparam("personname",true));

        myextend.ajaxPost_simple(webglobal.services.GetOneChaseVisitRecord, dictionary, web_list_callback, true);
    ...
    function save(sender) {
        require(["base_usercontrol", "jquery", "common", "jextend", "web_global", "easyui"], function (base) {
            ...
            if (dataparam == "" || dataparam == undefined)
                return;
            myextend.ajaxPost(webglobal.services.SaveChaseVisitRecord, dataparam, web_list_callback, web_list_bef_callback, web_list_com_callback, web_list_err_callback, true);
